...
def create_planning_map(center_gcj: List[float], points_gcj: Dict, 
    ...
    if planned_path and len(planned_path) > 1:
        ...
        folium.PolyLine(path_locations, color=line_color, weight=4, opacity=0.9).add_to(m)
        
        for point in planned_path[1:-1]:
            folium.CircleMarker([point[1], point[0]], radius=7, color=line_color, 
                              fill=True, fill_color="white", fill_opacity=0.9).add_to(m)
    ...
    if points_gcj.get('A') and points_gcj.get('B'):
        ...
        folium.PolyLine([[points_gcj['A'][1], points_gcj['A'][0]], 
                      ...
                      color=color, weight=2, opacity=0.5, dash_array=dash).add_to(m)
...
def main():
    ...
    if page == "航线规划":
        ...
        if straight_blocked:
            st.warning(f"有 {high_obstacles} 个障碍物高于飞行高度({flight_alt}m)，已自动规划避障路径")
        else:
        ...
        with col1:
            ...
            with st.expander("飞行控制", expanded=True):
                ...
                with col_met3:
                    st.metric("安全半径", f"{safety_radius} 米")
                ...
                with col_btn1:
                    if st.button("开始飞行", use_container_width=True, type="primary"):
                ...
                with col_btn2:
                    if st.button("停止飞行", use_container_width=True):
                        st.session_state.simulation_running = False
    ...
    # ==================== 飞行监控页面 ====================
        ...
        st.caption(f"当前飞行高度: {flight_alt} 米 | 避障策略: {st.session_state.current_direction} | 安全半径: {safety_radius} 米")
        ...
        if st.session_state.heartbeat_sim.history:
            ...
            col6.metric("卫星", latest['satellites'])
            ...
            if latest.get('safety_violation', False):
                st.error("警告：无人机进入安全半径危险区域！请立即检查！")
            ...
            for obs in st.session_state.obstacles_gcj:
                ...
                if coords and len(coords) >= 3:
                    ...
                    folium.Polygon([[c[1], c[0]] for c in coords], 
                                  color=color, weight=2, fill=True, fill_opacity=0.3).add_to(monitor_map)
    ...
    # ==================== 障碍物管理页面 ====================
        ...
        st.subheader("数据管理")
        st.info(f"当前共 {len(st.session_state.obstacles_gcj)} 个障碍物 | 安全半径: {safety_radius} 米")
        ...
        if "selected_obstacles" not in st.session_state:
            st.session_state.selected_obstacles = [False] * len(st.session_state.obstacles_gcj)
        
        with col_batch1:
            select_all = st.checkbox("全选所有障碍物")
            if select_all:
                st.session_state.selected_obstacles = [True] * len(st.session_state.obstacles_gcj)
            else:
                # 只有当用户明确取消全选时才重置
                if any(st.session_state.selected_obstacles):
                    pass
        
        with col_batch2:
            if st.button("批量删除选中", use_container_width=True, type="primary"):
                selected_indices = [i for i, selected in enumerate(st.session_state.selected_obstacles) if selected]
                if selected_indices:
                    # 从后往前删除
        ...
        with col_batch3:
            batch_height = st.number_input("批量高度", min_value=1, max_value=200, value=30, step=5, key="batch_height", label_visibility="collapsed")
        ...
        with col_list:
            if st.session_state.obstacles_gcj:
                ...
                for i, obs in enumerate(st.session_state.obstacles_gcj):
                    ...
                    with st.container():
                        col_check, col_info, col_edit, col_del = st.columns([0.3, 2, 1, 0.5])
                        
                        with col_check:
                            selected = st.checkbox("", value=st.session_state.selected_obstacles[i] if i < len(st.session_state.selected_obstacles) else False, 
                                                  key=f"select_{i}")
                            st.session_state.selected_obstacles[i] = selected
                        
                        with col_info:
                            st.markdown(f"**{color} {name}**")
                            st.caption(f"高度: {height}m | 顶点: {len(obs.get('polygon', []))}个")
                        ...
                        with col_del:
                            if st.button("删除", key=f"del_{i}"):
                                st.session_state.obstacles_gcj.pop(i)
                                st.session_state.selected_obstacles.pop(i) if i < len(st.session_state.selected_obstacles) else None
                                if st.session_state.auto_backup:
                                    save_obstacles(st.session_state.obstacles_gcj)
                                st.session_state.planned_path = create_avoidance_path(
                                    st.session_state.points_gcj['A'],
