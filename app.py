...
def point_to_segment_distance_deg(point: List[float], 
...
class HeartbeatSimulator:
    ...
    def update_and_generate(self, obstacles_gcj: List[Dict]) -> Optional[HeartbeatData]:
        ...
        base_speed = config.BASE_SPEED_MPS
...
def create_planning_map(center_gcj: List[float], points_gcj: Dict, 
    ...
    if planned_path and len(planned_path) > 1:
        ...
        if "向左" in direction:
            ...
            line_color = "green"
        folium.PolyLine(path_locations, color=line_color, weight=5, 
    ...
    if points_gcj.get('A') and points_gcj.get('B'):
        if not straight_blocked:
            folium.PolyLine([[points_gcj['A'][1], points_gcj['A'][0]], 
                           ...
                           dash_array='5, 5', popup="直线航线").add_to(m)
        else:
            folium.PolyLine([[points_gcj['A'][1], points_gcj['A'][0]], 
                            [points_gcj['B'][1], points_gcj['B'][0]]], 
                           color="gray", weight=2, opacity=0.4, 
                           dash_array='5, 5', popup="⚠️ 直线被阻挡").add_to(m)
    
    # 绘制安全半径
    if drone_pos:
        folium.Circle(radius=safety_radius, 
    ...
    if flight_history and len(flight_history) > 1:
        ...
        if len(trail) > 1:
            folium.PolyLine(trail, color="orange", weight=2, 
                           opacity=0.6, popup="历史轨迹").add_to(m)
...
def render_path_strategy(flight_alt: float):
    ...
    directions = ["最佳航线", "向左绕行", "向右绕行"]
    icons = ["🔄", "⬅️", "➡️"]
    colors = ["primary", "secondary", "secondary"]
    
    for col, direction, icon, color in zip([col_dir1, col_dir2, col_dir3], directions, icons, colors):
        with col:
            if st.button(f"{icon} {direction}", use_container_width=True,
                        type=color if st.session_state.current_direction == direction else "secondary"):
                st.session_state.current_direction = direction
                st.session_state.planned_path = create_avoidance_path(
                    st.session_state.points_gcj['A'], st.session_state.points_gcj['B'],
                    st.session_state.obstacles_gcj, flight_alt, direction,
                    st.session_state.safety_radius
                )
                st.success(f"已切换到{direction}模式")
                st.rerun()
...
def render_flight_monitoring_page(map_type: str, flight_alt: float, drone_speed: int):
    ...
    st.caption(f"✈️ 当前飞行高度: {flight_alt} 米 | 🧭 避障策略: {st.session_state.current_direction} | 🛡️ 安全半径: {st.session_state.safety_radius} 米")
    ...
    if st.session_state.heartbeat_sim.history:
        display_heartbeat_data(flight_alt, drone_speed)
    else:
...
def display_heartbeat_data(flight_alt: float, drone_speed: int):
    """显示心跳包数据"""
    ...
    # 进度条
    st.progress(latest.progress, text=f"✈️ 飞行进度: {latest.progress*100:.1f}%")
    
    if latest.arrived:
        st.success("🎉 无人机已到达目的地！飞行任务完成！")
    
    # 实时地图
    display_monitor_map(latest, flight_alt)
    
    # 数据图表
    display_flight_charts()
    
    # 历史记录表格
    display_flight_history()

def display_monitor_map(latest, flight_alt: float):
    """显示监控地图"""
    st.subheader("📍 实时位置")
    ...
    # 绘制安全半径
    folium.Circle(radius=st.session_state.safety_radius, 
    ...
    # 绘制当前位置
    folium.Marker([latest.lat, latest.lng], popup="当前位置", 
...
def display_flight_history():
    """显示飞行历史记录"""
    st.subheader("📋 历史心跳记录")
    history_df = st.session_state.heartbeat_sim.export_flight_data().head(10)
...
def render_obstacle_management_page(flight_alt: float):
    ...
    # 批量操作
    render_batch_operations(flight_alt)
...
def render_batch_operations(flight_alt: float):
    """渲染批量操作面板"""
    st.subheader("🎯 批量操作")
...
def render_obstacle_map_view(flight_alt: float):
    ...
    # 添加起点和终点标记
    folium.Marker([config.DEFAULT_A_GCJ[1], config.DEFAULT_A_GCJ[0]], 
                 popup="起点", icon=folium.Icon(color='green', icon='play', prefix='fa')).add_to(obs_map)
    folium.Marker([config.DEFAULT_B_GCJ[1], config.DEFAULT_B_GCJ[0]], 
...
def main():
    ...
    # 处理安全半径变化
    if 'safety_radius' in st.session_state and st.session_state.safety_radius != st.session_state.get('prev_safety_radius', 0):
        st.session_state.prev_safety_radius = st.session_state.safety_radius
        st.rerun()
