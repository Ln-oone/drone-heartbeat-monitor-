        ...
        if obs_height > flight_altitude:
            ...
            if coords and len(coords) >= 3:
                ...
                if bounds:
                    offset = 0.0002
                    # 添加四个方向的绕行点
                    waypoints.append([bounds['min_lng'] - offset, bounds['center_lat']])
                    waypoints.append([bounds['max_lng'] + offset, bounds['center_lat']])
                    waypoints.append([bounds['center_lng'], bounds['min_lat'] - offset])
                    waypoints.append([bounds['center_lng'], bounds['max_lat'] + offset])
...
def main():
    ...
    if page == "🗺️ 航线规划":
        ...
        with col1:
            ...
            st.metric("当前策略", st.session_state.avoid_direction)
        ...
        with col2:
            st.subheader("🗺️ 规划地图")
            if straight_blocked:
                st.caption(f"🟢 绿色=最佳航线 | 🟣 紫色=向左绕行 | 🟠 橙色=向右绕行\n当前: {st.session_state.avoid_direction}")
            else:
                st.caption("✅ 直线畅通")
