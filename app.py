...
def find_safe_offset_point(obs_polygon, direction, safety_radius_meters, lat=32.23):
    """根据方向找到安全的绕行点"""
    offset_lng, offset_lat = meters_to_deg(safety_radius_meters * 1.5, lat)
    bounds = get_polygon_bounds(obs_polygon)
    
    if direction == "left":  # 向左绕行（从左侧绕过）
        # 取障碍物左侧（最小经度）向左偏移
        offset_x = bounds['min_lng'] - offset_lng
        # 取中间纬度
        offset_y = bounds['center_lat']
        return [offset_x, offset_y]
    else:  # 向右绕行（从右侧绕过）
        # 取障碍物右侧（最大经度）向右偏移
        offset_x = bounds['max_lng'] + offset_lng
        offset_y = bounds['center_lat']
        return [offset_x, offset_y]

def find_waypoints_around_obstacle(start, end, obs_polygon, safety_radius_meters, side, lat=32.23):
    """
    围绕障碍物生成绕行点（直线连接，点自由排列）
    返回: [绕行点1, 绕行点2, ...] 至少2个点
    """
    offset_lng, offset_lat = meters_to_deg(safety_radius_meters * 1.5, lat)
    bounds = get_polygon_bounds(obs_polygon)
    ...
    if side == "left":  # 向左绕行（从左侧绕过）
        # 绕行点1：障碍物左上方
        wp1_x = bounds['min_lng'] - offset_lng
        wp1_y = bounds['max_lat'] + offset_lat
        waypoints.append([wp1_x, wp1_y])
        
        # 绕行点2：障碍物左侧中间
        wp2_x = bounds['min_lng'] - offset_lng
        wp2_y = bounds['center_lat']
        waypoints.append([wp2_x, wp2_y])
        
        # 绕行点3：障碍物左下方
        wp3_x = bounds['min_lng'] - offset_lng
        wp3_y = bounds['min_lat'] - offset_lat
        waypoints.append([wp3_x, wp3_y])
        
    else:  # 向右绕行（从右侧绕过）
        # 绕行点1：障碍物右上方
        wp1_x = bounds['max_lng'] + offset_lng
        wp1_y = bounds['max_lat'] + offset_lat
        waypoints.append([wp1_x, wp1_y])
        
        # 绕行点2：障碍物右侧中间
        wp2_x = bounds['max_lng'] + offset_lng
        wp2_y = bounds['center_lat']
        waypoints.append([wp2_x, wp2_y])
        
        # 绕行点3：障碍物右下方
        wp3_x = bounds['max_lng'] + offset_lng
        wp3_y = bounds['min_lat'] - offset_lat
        waypoints.append([wp3_x, wp3_y])
    
    return waypoints

def find_straight_path_around_obstacles(start, end, obstacles_gcj, flight_altitude, safety_radius_meters, side="best"):
    """
    使用直线绕行，生成避障路径
    side: "left" = 向左绕行, "right" = 向右绕行, "best" = 自动选择
    """
    ...
    if side == "best":
        left_path = find_straight_path_around_obstacles(start, end, obstacles_gcj, flight_altitude, safety_radius_meters, "left")
        right_path = find_straight_path_around_obstacles(start, end, obstacles_gcj, flight_altitude, safety_radius_meters, "right")
    ...
    for obs in blocking_obs:
        ...
        # 生成绕行点
        waypoints = find_waypoints_around_obstacle(
...
def find_left_path(start, end, obstacles_gcj, flight_altitude, safety_radius=5):
    """向左绕行（从障碍物左侧绕过）"""
    return find_straight_path_around_obstacles(start, end, obstacles_gcj, flight_altitude, safety_radius, "left")

def find_right_path(start, end, obstacles_gcj, flight_altitude, safety_radius=5):
    """向右绕行（从障碍物右侧绕过）"""
    return find_straight_path_around_obstacles(start, end, obstacles_gcj, flight_altitude, safety_radius, "right")

def find_best_path(start, end, obstacles_gcj, flight_altitude, safety_radius=5):
    """最佳航线（自动选择较短的路径）"""
    return find_straight_path_around_obstacles(start, end, obstacles_gcj, flight_altitude, safety_radius, "best")
...
def create_planning_map(center_gcj, points_gcj, obstacles_gcj, flight_history=None, planned_path=None, map_type="satellite", straight_blocked=True, flight_altitude=50, drone_pos=None, direction="最佳航线", safety_radius=5):
    ...
    if planned_path and len(planned_path) > 1:
        ...
        if "向左" in direction:
            ...
            line_color = "green"
        folium.PolyLine(path_locations, color=line_color, weight=5, opacity=0.9, popup=f"✈️ {direction}（直线绕行）").add_to(m)
        
        # 标记绕行点
        for i, point in enumerate(planned_path[1:-1]):
            folium.CircleMarker([point[1], point[0]], radius=5, color=line_color, fill=True, fill_color="white", fill_opacity=0.8, popup=f"绕行点 {i+1}").add_to(m)
...
def main():
    ...
    if page == "🗺️ 航线规划":
        ...
        st.caption("✨ **绕行方式**：使用直线连接绕行点，点自由排列，不接触障碍物")
        ...
        with col1:
            ...
            st.caption("💡 **说明**：向左绕行 = 从障碍物左侧绕过 | 向右绕行 = 从障碍物右侧绕过")
            
            # 显示路径信息
            if st.session_state.planned_path:
                waypoint_count = len(st.session_state.planned_path) - 2
                st.info(f"📍 绕行点数量: **{waypoint_count}** 个（每个障碍物3个绕行点）")
            ...
            if st.button("🔄 重新规划路径", use_container_width=True):
                ...
                st.success(f"已按照「{st.session_state.current_direction}」规划直线绕行路径")
                st.rerun()
            ...
            with col_btn1:
                if st.button("▶️ 开始飞行", use_container_width=True):
                    if st.session_state.planned_path and len(st.session_state.planned_path) >= 2:
                        ...
                        st.session_state.flight_history = []
                        st.success(f"🚁 飞行已开始！按照「{st.session_state.current_direction}」直线绕行路径飞行")
                    else:
                        st.error("请先规划路径")
        ...
        with col2:
            ...
            if straight_blocked:
                ...
                st.caption("✨ 路径使用**直线连接**绕行点，点自由排列")
            else:
        ...
        if st.session_state.heartbeat_sim.history:
            ...
            if st.session_state.planned_path and len(st.session_state.planned_path) > 1:
                if "向左" in st.session_state.current_direction:
                    ...
                    line_color = "green"
                folium.PolyLine([[p[1], p[0]] for p in st.session_state.planned_path], color=line_color, weight=3, opacity=0.7, popup="规划路径（直线绕行）").add_to(monitor_map)
    ...
    # ==================== 障碍物管理页面 ====================
        ...
        st.info(f"当前共 **{len(st.session_state.obstacles_gcj)}** 个障碍物 | 🛡️ 安全半径: {safety_radius} 米 | 🔄 绕行方式: 直线绕行")
