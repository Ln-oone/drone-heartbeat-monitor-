...
def find_left_path(start: List[float], end: List[float], 
    ...
    # 获取障碍物边界
    min_lng_all = min(p[0] for p in all_points)
    max_lng_all = max(p[0] for p in all_points)
    min_lat_all = min(p[1] for p in all_points)
    max_lat_all = max(p[1] for p in all_points)
    
    # 向左绕行：取障碍物最左侧，再向左偏移作为绕行经度
    left_x = min_lng_all - offset_lng
    
    # 判断起点和终点的Y坐标关系，决定向上还是向下绕过
    if start[1] > end[1]:
        # 起点在上方，终点在下方 -> 从下方绕过
        bypass_y = max_lat_all + offset_lat * 2
        ...
        # 起点在下方，终点在上方 -> 从上方绕过
        bypass_y = min_lat_all - offset_lat * 2
    ...
    for retry in range(max_retries):
        ...
        for i in range(len(unique_path) - 1):
            if line_intersects_polygon(unique_path[i], unique_path[i+1], all_points):
                collision_found = True
                # 增加偏移量
                larger_offset_lng, larger_offset_lat = meters_to_deg(safety_radius * (3 + retry * 2))
                new_left_x = min_lng_all - larger_offset_lng
                
                if start[1] > end[1]:
                    new_bypass_y = max_lat_all + larger_offset_lat * 2
                else:
                    new_bypass_y = min_lat_all - larger_offset_lat * 2
                
                # 更新路径
                unique_path = [
                    start.copy(),
                    [new_left_x, start[1]],
                    [new_left_x, new_bypass_y],
                    [end[0], new_bypass_y],
                    end.copy()
                ]
                break
    ...
    # 计算路径长度
    total_length = sum(distance(unique_path[i], unique_path[i+1]) for i in range(len(unique_path)-1)) * 111000
    
    st.sidebar.caption(f"📐 向左绕行路径长度: {total_length:.0f}米")
...
def render_flight_monitoring_page(map_type: str, flight_alt: float, drone_speed: int):
    ...
    if st.session_state.heartbeat_sim.history:
        ...
        with col4:
            st.metric("💨 当前速度", f"{latest.speed} m/s", delta=f"{drone_speed}%")
        with col5:
            st.metric("📏 剩余距离", f"{latest.remaining_distance:.0f} m")
        
        st.markdown("### 🔧 设备状态")
        col6, col7, col8, col9, col10 = st.columns(5)
        
        with col6:
            st.metric("🔋 电池电压", f"{latest.voltage} V")
        with col7:
            st.metric("🛰️ 卫星数量", f"{latest.satellites} 颗")
        with col8:
            st.metric("🎯 任务进度", f"{progress_percent}%")
        with col9:
...
def display_monitor_map(map_type: str, latest: HeartbeatData, flight_alt: float):
    ...
    with col_data3:
        if st.session_state.obstacles_gcj:
            config_data = {'obstacles': st.session_state.obstacles_gcj, 'count': len(st.session_state.obstacles_gcj),
                          'export_time': datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 'version': 'v13.1'}
            json_str = json.dumps(config_data, ensure_ascii=False, indent=2)
            st.download_button("📥 导出配置", json_str, f"obstacles_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json", "application/json", use_container_width=True)
        else:
