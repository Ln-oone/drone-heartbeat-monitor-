...
def find_left_path(start: List[float], end: List[float], obstacles_gcj: List[Dict], flight_altitude: float, safety_radius: float = 5) -> List[List[float]]:
    """
    向左绕行：从顶部绕过障碍物
    第1段（起点→点1）：垂直向上飞到障碍物顶部上方
    第2段（点1→点2）：水平向右飞过障碍物顶部
    第3段（点2→终点）：垂直向下到终点
    """
    ...
    # ========== 修改点：一号绕行点的纬度 ==========
    # 原代码：飞到障碍物顶部上方很远处（障碍物高度的3倍 + 安全偏移）
    # point1 = [start[0], max_lat + obstacle_height * 3 + safe_lat * 5]
    
    # 新代码：只向上移动0.001度（约111米），使其更贴合障碍物顶部
    # 计算基础高度：障碍物最高点 + 少量安全余量
    base_height = max_lat + safe_lat * 2  # 基础安全高度
    
    # 应用用户要求的偏移：纬度增加0.001度（约111米）
    CUSTOM_LAT_OFFSET = 0.001  # 向上移动0.001度
    
    point1 = [
        start[0],  # X坐标不变（经度）
        base_height + CUSTOM_LAT_OFFSET  # 纬度 = 障碍物最高点 + 安全余量 + 自定义偏移
    ]
    # ============================================
    ...
    # 向右飞过整个障碍物宽度 + 额外距离
    point2 = [
        max_lng + obstacle_height * 1.5 + safe_lng * 2,  # 适当减少向右偏移距离
        point1[1]  # Y坐标不变（保持高度）
    ]
